...
QAlgoWrapper::QAlgoWrapper(std::shared_ptr<OptimAlgoI<std::shared_ptr<DataHolder>>> algo) : algo_(algo) {
	stepIdx_ = 0;
	mod_ = 2;
	isDone_ = false;
}
std::shared_ptr<OptimAlgoI<std::shared_ptr<DataHolder>>> QAlgoWrapper::getAlgo()
{
	return algo_;
}

void QAlgoWrapper::setAlgorithm(std::shared_ptr<OptimAlgoI<std::shared_ptr<DataHolder>>> alg)
{
	algo_ = alg;
}
