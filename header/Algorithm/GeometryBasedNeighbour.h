...
public:
	GeometryBasedNeighbour(std::shared_ptr<DataHolderT<Data>> data, std::shared_ptr<DataHolderT<Data>> currentBest, std::shared_ptr<InitialSolutionI<Data>> initSol);
...
template<class Data>
inline GeometryBasedNeighbour<Data>::GeometryBasedNeighbour(std::shared_ptr<DataHolderT<Data>> data, std::shared_ptr<DataHolderT<Data>> currentBest, std::shared_ptr<InitialSolutionI<Data>> initSol) : GeometryBasedNeighbourI<Data>(data, currentBest, initSol)
{
	rectPos_ = 0;
	boxPos_ = -1;
...
template<>
inline float GeometryBasedNeighbour<std::shared_ptr<DataHolder>>::optimize() {
	return this->findNeighbour(true);
}


template<>
inline void GeometryBasedNeighbour<std::shared_ptr<DataHolder>>::beforeScoreCalculation(std::vector<class RectangleHolder*>* rectangles, std::vector<std::shared_ptr<BoundingBox>>& bBoxList, bool isTransgressionRect) {
}
